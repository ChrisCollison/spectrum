...
def getrdkitDescriptors(mol, missingVal=None):
...
def get_rdkit_values(data: pd.DataFrame) -> pd.DataFrame:
    """
    Calculates the rdkit descriptor values to the provided dataframe. The Mordred descriptors are calculated using the SMILES representation of the molecules and are not based on experimental data. The list of descriptors can be found [here](https://mordred-descriptor.github.io/documentation/master/descriptors.html). This can take a while to run depending on the size of the dataset and the number of features.
    ...
    # Calculate Mordred descriptors
    rdkit_calc = [getrdkitDescriptors(m) for m in molecules]

    # Create dataframe of rdkit calculated descriptors values
    print(
        f"Calculating {(len(Descriptors._descList))} rdkit descriptors for each row...\n"
    )
    rdkit_data = pd.DataFrame(rdkit_calc)
