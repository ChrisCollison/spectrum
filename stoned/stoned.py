...
def substructure_preserver(mol, substructure_smarts: str):
    """
    Check for substructure violates
    Return True: contains no substructure violation
    Return False: Contains a substructure violation
    """        
    
    # added this on 05/01/2023 to address an error when no substructure is passed
    if substructure_smarts is None:
        return True

    # if mol.HasSubstructMatch(Chem.MolFromSmarts('[#6]1-[#6]2:[#6]:[#6]:[#6]:[#6]:[#6]:2-[#6]2:[#6]:[#6]:[#6]:[#6]:[#6]:2-1')) == True: # fluorene
    # if mol.HasSubstructMatch(Chem.MolFromSmarts('[#6]1=[#6](-[#8-])-[#6]-[#6]-1=[#8]')) == True: # squaraine  
    if mol.HasSubstructMatch(Chem.MolFromSmarts(substructure_smarts)) == True: # smarts_substructure 
    # if mol.HasSubstructMatch(rdkit.Chem.MolFromSmarts(Smarts_substructure)) == True:
        return True # The molecule has the substructure! 
    else: 
        return False # Molecule does not have substructure!


def mutate_selfie(selfie, max_molecules_len, alphabet, substructure_smarts, write_fail_cases=False):
    ...
    while not valid:
        ...
        if len(selfie_mutated_chars) > max_molecules_len or smiles == "" or substructure_preserver(_, substructure_smarts)==False:
            done = False
        if done:
            valid = True
        else:
...
def get_mutated_SELFIES(selfies_ls, num_mutations, alphabet, substructure_smarts):
    ...
    for _ in range(num_mutations):
        ...
        for str_ in selfies_ls:
            ...
            selfie_mutated = mutate_selfie(str_, max_molecules_len, alphabet, substructure_smarts)
